...
$bookstmt = $pdo->prepare("SELECT book_name, book_price, order_detail_qty FROM order_details ordet, books pl WHERE ordet.book_id = pl.book_id AND order_id = ? ORDER BY order_detail_unit_price");
$bookstmt->execute([$transid]);

$bookquantityarray = array();
$booktotalarray = array();
$bookpricearray = array();
$booknamearray = array();

while ($row = $bookstmt->fetch(PDO::FETCH_ASSOC)) {
  array_push($bookquantityarray, $row['order_detail_qty']);
  array_push($bookpricearray, $row['book_price']);
  array_push($booknamearray, $row['book_name']);
}

$len = count($booknamearray);
$drop = $len + 1;
$totalprice = 0;
$totalquantity = 0;
        ...
        for ($rait = 0; $rait <= $drop; $rait++) {
          for ($don = 0; $don <= 4; $don++) {
            if ($rait == 0) {
              ...
              echo '<td>' . $booknamearray[$rait - 1] . '</td>';
            } else if ($don == 2 && $rait != 0 && $rait != $drop) { // kolom 2 untuk harga produk
              echo '<td>Rp' . number_format($bookpricearray[$rait - 1]) . '</td>';
            } else if ($don == 3 && $rait != 0 && $rait != $drop) { // kolom 3 untuk kuantitas produk
              echo '<td>' . $bookquantityarray[$rait - 1] . '</td>';
              $totalquantity += $bookquantityarray[$rait - 1];
            } else {
              if ($rait != $len) { // kolom 4 untuk sub-total
                echo '<td>Rp' . number_format($bookpricearray[$rait - 1] * $bookquantityarray[$rait - 1]) . '</td>';
                $totalprice = $totalprice + $bookpricearray[$rait - 1] * $bookquantityarray[$rait - 1];
              } else if ($rait == $len) {
                echo '<td>Rp' . number_format($bookpricearray[$rait - 1] * $bookquantityarray[$rait - 1]) . '</td><tbody>';
                $totalprice = $totalprice + $bookpricearray[$rait - 1] * $bookquantityarray[$rait - 1];
              } else {
                echo '<td> </td>';
              }
